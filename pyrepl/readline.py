...
from pyrepl import commands, historical_reader
...
class _ReadlineWrapper:
    ...
    def __init__(self, f_in=None, f_out=None):
        self.f_in = f_in if f_in is not None else os.dup(sys.stdin.fileno())
        self.f_out = f_out if f_out is not None else os.dup(sys.stdout.fileno())
